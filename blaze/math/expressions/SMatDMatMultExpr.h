      ...
      for( size_t i=0UL; i<A.rows(); ++i )
      {
         ...
         for( size_t k=0UL; k<kend; k+=4UL ) {
            ...
            for( size_t j=0UL; j<N; j+=IT::size ) {
               (~C).store( i, j, (~C).get(i,j) + v1 * B.get(i1,j) + v2 * B.get(i2,j) + v3 * B.get(i3,j) + v4 * B.get(i4,j) );
            }
         }
         ...
         for( ; element!=end; ++element ) {
            ...
            for( size_t j=0UL; j<N; j+=IT::size ) {
               (~C).store( i, j, (~C).get(i,j) + v1 * B.get(i1,j) );
            }
         }
      }
   }
...
struct SubmatrixExprTrait< SMatDMatMultExpr<MT1,MT2> >
{
 public:
   //**********************************************************************************************
