      ...
      selectAssignKernel( VT1& y, const VT2& x, const MT1& A )
   {
      ...
      if( iend > 3UL )
      {
         ...
         for( size_t j=0UL; j<N; j+=IT::size ) {
            y.store( j, v1 * A.get(i1,j) + v2 * A.get(i2,j) + v3 * A.get(i3,j) + v4 * A.get(i4,j) );
         }
      }
      else
      {
      ...
      for( size_t i=(iend>3UL)?(4UL):(1UL); (i+4UL)<=iend; i+=4UL )
      {
         ...
         for( size_t j=0UL; j<N; j+=IT::size ) {
            y.store( j, y.get(j) + v1 * A.get(i1,j) + v2 * A.get(i2,j) + v3 * A.get(i3,j) + v4 * A.get(i4,j) );
         }
      }
      for( ; element!=end; ++element )
      {
         ...
         for( size_t j=0UL; j<N; j+=IT::size ) {
            y.store( j, y.get(j) + v1 * A.get(i1,j) );
         }
      }
   }
   /*! \endcond */
...
struct SubvectorExprTrait< TSVecDMatMultExpr<VT,MT> >
{
 public:
   //**********************************************************************************************
