   ...
   */
      ...
      selectAssignKernel( DenseMatrix<MT3,true>& C, const MT4& A, const MT5& B )
   {
      ...
      for( size_t j=0UL; j<B.columns(); ++j )
      {
         ...
         const size_t kend( B.nonZeros(j) & size_t(-4) );
      ...
      for( size_t j=0UL; j<B.columns(); ++j )
      {
         ...
         for( size_t k=0UL; k<kend; k+=4UL ) {
            ...
            for( size_t i=0UL; i<M; i+=IT::size ) {
               store( &(~C)(i,j), load( &(~C)(i,j) ) + A.get(i,j1) * v1 + A.get(i,j2) * v2 + A.get(i,j3) * v3 + A.get(i,j4) * v4 );
            }
         }
         ...
         for( ; element!=end; ++element ) {
            ...
            for( size_t i=0UL; i<M; i+=IT::size ) {
               store( &(~C)(i,j), load( &(~C)(i,j) ) + A.get(i,j1) * v1 );
            }
         }
      }
   }
...
//*************************************************************************************************
/*! \cond BLAZE_INTERNAL */
/*!\brief Creating a view on a specific row of the given transpose dense matrix/transpose sparse
//        matrix multiplication.
// \ingroup views
//
// \param dm The constant transpose dense matrix/transpose sparse matrix multiplication.
...
template< typename MT1    // Type of the left-hand side dense matrix
        , typename MT2 >  // Type of the right-hand side sparse matrix
inline typename RowExprTrait< TDMatTSMatMultExpr<MT1,MT2> >::Type
   row( const TDMatTSMatMultExpr<MT1,MT2>& dm, size_t index )
{
   BLAZE_FUNCTION_TRACE;
...
template< typename MT1    // Type of the left-hand side dense matrix
        , typename MT2 >  // Type of the right-hand side sparse matrix
inline typename ColumnExprTrait< TDMatTSMatMultExpr<MT1,MT2> >::Type
   column( const TDMatTSMatMultExpr<MT1,MT2>& dm, size_t index )
{
   BLAZE_FUNCTION_TRACE;
