                        ...
                        , private Expression
{
 ...
 private:
   ...
   */
   ...
   // \return void
   //
   // This function implements the default assignment of a dense matrix-transpose dense matrix
      ...
      selectDefaultAssignKernel( DenseMatrix<MT3,false>& C, const MT4& A, const MT5& B )
   {
      ...
      for( ; (i+2UL) <= M; i+=2UL ) {
         ...
         for( ; (j+4UL) <= N; j+=4UL ) {
            ...
            for( size_t k=0UL; k<K; k+=IT::size ) {
               const IntrinsicType a1( A.get(i    ,k) );
               const IntrinsicType a2( A.get(i+1UL,k) );
               const IntrinsicType b1( B.get(k,j    ) );
               const IntrinsicType b2( B.get(k,j+1UL) );
               const IntrinsicType b3( B.get(k,j+2UL) );
               const IntrinsicType b4( B.get(k,j+3UL) );
               xmm1 = xmm1 + a1 * b1;
               xmm2 = xmm2 + a1 * b2;
               xmm3 = xmm3 + a1 * b3;
               xmm4 = xmm4 + a1 * b4;
      ...
      if( i < M ) {
         ...
         for( ; (j+4UL) <= N; j+=4UL ) {
            ...
            for( size_t k=0UL; k<K; k+=IT::size ) {
               ...
               xmm2 = xmm2 + a1 * B.get(k,j+1UL);
               xmm3 = xmm3 + a1 * B.get(k,j+2UL);
               xmm4 = xmm4 + a1 * B.get(k,j+3UL);
            }
            (~C)(i,j    ) = sum( xmm1 );
            (~C)(i,j+1UL) = sum( xmm2 );
            (~C)(i,j+2UL) = sum( xmm3 );
      ...
      for( ; (i+4UL) <= M; i+=4UL ) {
         ...
         for( ; (j+2UL) <= N; j+=2UL ) {
            ...
            for( size_t k=0UL; k<K; k+=IT::size ) {
               const IntrinsicType a1( A.get(i    ,k) );
               const IntrinsicType a2( A.get(i+1UL,k) );
               const IntrinsicType a3( A.get(i+2UL,k) );
               const IntrinsicType a4( A.get(i+3UL,k) );
               const IntrinsicType b1( B.get(k,j    ) );
               const IntrinsicType b2( B.get(k,j+1UL) );
               xmm1 = xmm1 + a1 * b1;
               xmm2 = xmm2 + a1 * b2;
         ...
         if( j < N ) {
            ...
            for( size_t k=0UL; k<K; k+=IT::size ) {
               ...
               xmm1 = xmm1 + A.get(i    ,k) * b1;
               xmm2 = xmm2 + A.get(i+1UL,k) * b1;
               xmm3 = xmm3 + A.get(i+2UL,k) * b1;
               xmm4 = xmm4 + A.get(i+3UL,k) * b1;
            }
            (~C)(i    ,j) = sum( xmm1 );
            (~C)(i+1UL,j) = sum( xmm2 );
            (~C)(i+2UL,j) = sum( xmm3 );
   ...
   */
...
#if BLAZE_BLAS_MODE
   /*! \cond BLAZE_INTERNAL */
   /*!\brief BLAS-based assignment of a dense matrix-transpose dense matrix multiplication for
   ...
   // \return void
   //
   // This function performs the dense matrix-transpose dense matrix multiplication for single
   ...
   */
   ...
   // \return void
   //
   // This function performs the dense matrix-transpose dense matrix multiplication for double
   ...
   */
   ...
   // \return void
   //
   // This function implements the default addition assignment of a dense matrix-transpose dense
   ...
   , private Expression
{
 ...
 private:
   ...
   */
   ...
   // \return void
   //
   // This function implements the default assignment of a scaled dense matrix-transpose dense
   // matrix multiplication expression to a dense matrix.
   */
   template< typename MT3    // Type of the left-hand side target matrix
           , typename MT4    // Type of the left-hand side matrix operand
   ...
   */
   ...
   // \return void
   //
   // This function performs the scaled dense matrix-transpose dense matrix multiplication for
   ...
   */
   ...
   //
   // This function performs the scaled dense matrix-transpose dense matrix multiplication for
   // double precision matrices based on the BLAS cblas_dgemm() function.
   */
   template< typename MT3    // Type of the left-hand side target matrix
           , typename MT4    // Type of the left-hand side matrix operand
           , typename MT5    // Type of the right-hand side matrix operand
   ...
   //
   // This function performs the scaled dense matrix-transpose dense matrix multiplication for
   // single precision complex matrices based on the BLAS cblas_cgemm() function.
   */
   template< typename MT3    // Type of the left-hand side target matrix
           , typename MT4    // Type of the left-hand side matrix operand
           , typename MT5    // Type of the right-hand side matrix operand
   ...
   //
   // This function performs the scaled dense matrix-transpose dense matrix multiplication for
   // double precision complex matrices based on the BLAS cblas_zgemm() function.
   */
   template< typename MT3    // Type of the left-hand side target matrix
           , typename MT4    // Type of the left-hand side matrix operand
           , typename MT5    // Type of the right-hand side matrix operand
   ...
   */
   ...
   // \return void
   //
   // This function implements the default addition assignment of a scaled dense matrix-transpose
   // dense matrix multiplication expression to a dense matrix.
   */
   template< typename MT3    // Type of the left-hand side target matrix
           , typename MT4    // Type of the left-hand side matrix operand
   ...
   */
   ...
   // \return void
   //
   // This function implements the default subtraction assignment of a scaled dense matrix-
   // transpose dense matrix multiplication expression to a dense matrix.
   */
   template< typename MT3    // Type of the left-hand side target matrix
           , typename MT4    // Type of the left-hand side matrix operand
...
struct DMatDVecMultTrait< DMatTDMatMultExpr<MT1,MT2>, VT >
{
 public:
   //**********************************************************************************************
   typedef typename DMatDVecMultTrait< MT1, typename TDMatDVecMultTrait<MT2,VT>::Type >::Type  Type;
   //**********************************************************************************************
...
/*! \cond BLAZE_INTERNAL */
template< typename MT1, typename MT2, typename VT >
struct DMatSVecMultTrait< DMatTDMatMultExpr<MT1,MT2>, VT >
{
 public:
   //**********************************************************************************************
   typedef typename DMatDVecMultTrait< MT1, typename TDMatSVecMultTrait<MT2,VT>::Type >::Type  Type;
   //**********************************************************************************************
...
struct TDVecDMatMultTrait< VT, DMatTDMatMultExpr<MT1,MT2> >
{
 public:
   //**********************************************************************************************
   typedef typename TDVecTDMatMultTrait< typename TDVecDMatMultTrait<VT,MT1>::Type, MT2 >::Type  Type;
   //**********************************************************************************************
...
/*! \cond BLAZE_INTERNAL */
template< typename VT, typename MT1, typename MT2 >
struct TSVecDMatMultTrait< VT, DMatTDMatMultExpr<MT1,MT2> >
{
 public:
   //**********************************************************************************************
   typedef typename TDVecTDMatMultTrait< typename TSVecDMatMultTrait<VT,MT1>::Type, MT2 >::Type  Type;
   //**********************************************************************************************
