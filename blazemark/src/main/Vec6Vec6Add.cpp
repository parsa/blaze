...
void estimateSteps( DenseRun& run )
{
   ...
   blaze::StaticVector<real,6UL,columnVector> init( 0.1 );
   std::vector< blaze::StaticVector<real,6UL,columnVector> > a( N, init ), b( N, init ), c( N );
...
// \return void
*/
void vec6vec6add( std::vector<DenseRun>& runs, Benchmarks benchmarks )
{
   std::cout << std::left;
