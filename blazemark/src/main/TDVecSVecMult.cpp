//=================================================================================================
/*!
//  \file src/blaze/TDVecSVecMult.cpp
//  \brief Source file for the dense vector/sparse vector inner product benchmark
//
...
#include <blazemark/blaze/TDVecSVecMult.h>
#include <blazemark/boost/TDVecSVecMult.h>
#include <blazemark/gmm/TDVecSVecMult.h>
...
// vector/sparse vector inner product benchmark
*/
...
//*************************************************************************************************
/*!\brief Dense vector/sparse vector inner product benchmark function.
//
// \param runs The specified benchmark runs.
// \param benchmarks The selection of benchmarks.
// \return void
*/
void tdvecsvecmult( std::vector<Run>& runs, Benchmarks benchmarks )
{
   std::cout << std::left;
   ...
   if( benchmarks.runBlaze ) {
      ...
      while( run != runs.end() ) {
         ...
         for( ; run!=runs.end(); ++run ) {
            ...
            run->setBlazeResult( blazemark::blaze::tdvecsvecmult( N, F, steps ) );
            const double mflops( ( 2U*F - 1U ) * steps / run->getBlazeResult() / 1E6 );
   ...
   if( benchmarks.runBoost ) {
      ...
      while( run != runs.end() ) {
         ...
         for( ; run!=runs.end(); ++run ) {
            ...
            const size_t F    ( run->getNonZeros() );
            const size_t steps( run->getSteps()    );
            run->setBoostResult( blazemark::boost::tdvecsvecmult( N, F, steps ) );
            const double mflops( ( 2U*F - 1U ) * steps / run->getBoostResult() / 1E6 );
            std::cout << "     " << std::setw(12) << N << mflops << std::endl;
         }
      }
   }
   ...
   if( benchmarks.runGMM ) {
      ...
      while( run != runs.end() ) {
         ...
         for( ; run!=runs.end(); ++run ) {
            ...
            const size_t F    ( run->getNonZeros() );
            const size_t steps( run->getSteps()    );
            run->setGMMResult( blazemark::gmm::tdvecsvecmult( N, F, steps ) );
            const double mflops( ( 2U*F - 1U ) * steps / run->getGMMResult() / 1E6 );
            std::cout << "     " << std::setw(12) << N << mflops << std::endl;
         }
      }
   }
...
//*************************************************************************************************
/*!\brief The main function for the dense vector/sparse vector inner product benchmark.
//
...
int main( int argc, char** argv )
{
   ...
   const std::string parameterFile( installPath + "/params/tdvecsvecmult.prm" );
   Parser<Run> parser;
   std::vector<Run> runs;
