//=================================================================================================
/*!
//  \file src/blaze/TMat3TMat3Mult.cpp
//  \brief Source file for the 3D transpose matrix/transpose matrix multiplication benchmark
//
...
#include <blazemark/armadillo/TMat3TMat3Mult.h>
#include <blazemark/blaze/TMat3TMat3Mult.h>
#include <blazemark/blitz/TMat3TMat3Mult.h>
#include <blazemark/boost/TMat3TMat3Mult.h>
#include <blazemark/eigen/TMat3TMat3Mult.h>
#include <blazemark/flens/TMat3TMat3Mult.h>
#include <blazemark/gmm/TMat3TMat3Mult.h>
#include <blazemark/mtl/TMat3TMat3Mult.h>
...
void estimateSteps( DenseRun& run )
{
   ...
   blaze::StaticMatrix<element_t,3UL,3UL,columnMajor> init( 0.1 );
   std::vector< blaze::StaticMatrix<element_t,3UL,3UL,columnMajor> > A( N, init ), B( N, init ), C( N );
...
//*************************************************************************************************
/*!\brief 3-dimensional transpose matrix/transpose matrix multiplication benchmark function.
//
// \param runs The specified benchmark runs.
// \param benchmarks The selection of benchmarks.
// \return void
*/
void tmat3tmat3mult( std::vector<DenseRun>& runs, Benchmarks benchmarks )
{
   std::cout << std::left;
   ...
   if( benchmarks.runBlaze ) {
      ...
      for( std::vector<DenseRun>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         ...
         run->setBlazeResult( blazemark::blaze::tmat3tmat3mult( N, steps ) );
   ...
   if( benchmarks.runBoost ) {
      ...
      for( std::vector<DenseRun>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setBoostResult( blazemark::boost::tmat3tmat3mult( N, steps ) );
   ...
   if( benchmarks.runBlitz ) {
      ...
      for( std::vector<DenseRun>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setBlitzResult( blazemark::blitz::tmat3tmat3mult( N, steps ) );
   ...
   if( benchmarks.runGMM ) {
      ...
      for( std::vector<DenseRun>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setGMMResult( blazemark::gmm::tmat3tmat3mult( N, steps ) );
         const double mflops( 45UL * steps / run->getGMMResult() / 1E6 );
         std::cout << "     " << std::setw(12) << N << mflops << std::endl;
      }
   }
#endif
   ...
   if( benchmarks.runArmadillo ) {
      ...
      for( std::vector<DenseRun>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setArmadilloResult( blazemark::armadillo::tmat3tmat3mult( N, steps ) );
         const double mflops( 45UL * steps / run->getArmadilloResult() / 1E6 );
         std::cout << "     " << std::setw(12) << N << mflops << std::endl;
      }
   }
#endif
   ...
   if( benchmarks.runFLENS ) {
      ...
      for( std::vector<DenseRun>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setFLENSResult( blazemark::flens::tmat3tmat3mult( N, steps ) );
         const double mflops( 45UL * steps / run->getFLENSResult() / 1E6 );
         std::cout << "     " << std::setw(12) << N << mflops << std::endl;
      }
   }
#endif
   ...
   if( benchmarks.runMTL ) {
      ...
      for( std::vector<DenseRun>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setMTLResult( blazemark::mtl::tmat3tmat3mult( N, steps ) );
   ...
   if( benchmarks.runEigen ) {
      ...
      for( std::vector<DenseRun>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setEigenResult( blazemark::eigen::tmat3tmat3mult( N, steps ) );
...
//*************************************************************************************************
/*!\brief The main function for the 3-dimensional transpose matrix/transpose matrix multiplication
//        benchmark.
//
...
int main( int argc, char** argv )
{
   ...
   const std::string parameterFile( installPath + "/params/tmat3tmat3mult.prm" );
   ...
   try {
      tmat3tmat3mult( runs, benchmarks );
   }
   catch( std::exception& ex ) {
