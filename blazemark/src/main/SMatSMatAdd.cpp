...
#include <blazemark/blaze/SMatSMatAdd.h>
#include <blazemark/boost/SMatSMatAdd.h>
#include <blazemark/eigen/SMatSMatAdd.h>
#include <blazemark/gmm/SMatSMatAdd.h>
#include <blazemark/mtl/SMatSMatAdd.h>
...
void estimateSteps( SparseRun& run )
{
   ...
   blaze::CompressedMatrix<real,rowMajor> A( N, N, N*F ), B( N, N, N*F ), C( N, N );
...
//*************************************************************************************************
/*!\brief Sparse matrix/sparse matrix addition benchmark function.
//
// \param runs The specified benchmark runs.
// \param benchmarks The selection of benchmarks.
// \return void
*/
void smatsmatadd( std::vector<SparseRun>& runs, Benchmarks benchmarks )
{
   std::cout << std::left;
   ...
   if( benchmarks.runBlaze ) {
      ...
      while( run != runs.end() ) {
         ...
         for( ; run!=runs.end(); ++run ) {
            ...
            run->setBlazeResult( blazemark::blaze::smatsmatadd( N, F, steps ) );
   ...
   if( benchmarks.runBoost ) {
      ...
      while( run != runs.end() ) {
         ...
         for( ; run!=runs.end(); ++run ) {
            ...
            const size_t F    ( run->getNonZeros() );
            const size_t steps( run->getSteps()    );
            run->setBoostResult( blazemark::boost::smatsmatadd( N, F, steps ) );
   ...
   if( benchmarks.runGMM ) {
      ...
      while( run != runs.end() ) {
         ...
         for( ; run!=runs.end(); ++run ) {
            ...
            const size_t F    ( run->getNonZeros() );
            const size_t steps( run->getSteps()    );
            run->setGMMResult( blazemark::gmm::smatsmatadd( N, F, steps ) );
   ...
   if( benchmarks.runMTL ) {
      ...
      while( run != runs.end() ) {
         ...
         for( ; run!=runs.end(); ++run ) {
            ...
            const size_t F    ( run->getNonZeros() );
            const size_t steps( run->getSteps()    );
            run->setMTLResult( blazemark::mtl::smatsmatadd( N, F, steps ) );
   ...
   if( benchmarks.runEigen ) {
      ...
      while( run != runs.end() ) {
         ...
         for( ; run!=runs.end(); ++run ) {
            ...
            const size_t F    ( run->getNonZeros() );
            const size_t steps( run->getSteps()    );
            run->setEigenResult( blazemark::eigen::smatsmatadd( N, F, steps ) );
            const double mflops( ( N*F ) * steps / run->getEigenResult() / 1E6 );
            std::cout << "     " << std::setw(12) << N << mflops << std::endl;
         }
      }
   }
...
//*************************************************************************************************
/*!\brief The main function for the sparse matrix/sparse matrix addition benchmark.
//
...
int main( int argc, char** argv )
{
   ...
   const std::string parameterFile( installPath + "/params/smatsmatadd.prm" );
