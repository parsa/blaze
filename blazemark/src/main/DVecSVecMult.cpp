//=================================================================================================
/*!
//  \file src/blaze/DVecSVecMult.cpp
//  \brief Source file for the dense vector/sparse vector multiplication benchmark
//
...
#include <blazemark/blaze/DVecSVecMult.h>
...
void estimateSteps( SparseRun& run )
{
   ...
   blaze::CompressedVector<element_t,columnVector> b( N, F ), c( N );
...
//*************************************************************************************************
/*!\brief Dense vector/sparse vector multiplication benchmark function.
//
// \param runs The specified benchmark runs.
// \param benchmarks The selection of benchmarks.
// \return void
*/
void dvecsvecmult( std::vector<SparseRun>& runs, Benchmarks benchmarks )
{
   std::cout << std::left;
   ...
   if( benchmarks.runBlaze ) {
      ...
      while( run != runs.end() ) {
         ...
         for( ; run!=runs.end(); ++run ) {
            ...
            run->setBlazeResult( blazemark::blaze::dvecsvecmult( N, F, steps ) );
   ...
   if( benchmarks.runBoost ) {
      ...
      while( run != runs.end() ) {
         ...
         for( ; run!=runs.end(); ++run ) {
            ...
            const size_t F    ( run->getNonZeros() );
            const size_t steps( run->getSteps()    );
            run->setBoostResult( blazemark::boost::dvecsvecmult( N, F, steps ) );
...
//*************************************************************************************************
/*!\brief The main function for the dense vector/sparse vector multiplication benchmark.
//
...
int main( int argc, char** argv )
{
   ...
   const std::string parameterFile( installPath + "/params/dvecsvecmult.prm" );
