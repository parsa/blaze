...
//  \brief Source file for the dense matrix/dense matrix addition benchmark
//
...
#include <blazemark/blitz/DMatDMatAdd.h>
...
void estimateSteps( DenseRun& run )
{
   ...
   blaze::DynamicMatrix<element_t,rowMajor> A( N, N, 0.1 ), B( N, N, 0.1 ), C( N, N );
...
//*************************************************************************************************
/*!\brief Dense matrix/dense matrix addition benchmark function.
//
// \param runs The specified benchmark runs.
// \param benchmarks The selection of benchmarks.
// \return void
*/
void dmatdmatadd( std::vector<DenseRun>& runs, Benchmarks benchmarks )
{
   std::cout << std::left;
   ...
   if( benchmarks.runClassic ) {
      ...
      for( std::vector<DenseRun>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         ...
         run->setClassicResult( blazemark::classic::dmatdmatadd( N, steps ) );
   ...
   if( benchmarks.runBlaze ) {
      ...
      for( std::vector<DenseRun>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setBlazeResult( blazemark::blaze::dmatdmatadd( N, steps ) );
   ...
   if( benchmarks.runBoost ) {
      ...
      for( std::vector<DenseRun>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setBoostResult( blazemark::boost::dmatdmatadd( N, steps ) );
   ...
   if( benchmarks.runMTL ) {
      ...
      for( std::vector<DenseRun>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setMTLResult( blazemark::mtl::dmatdmatadd( N, steps ) );
   ...
   if( benchmarks.runEigen ) {
      ...
      for( std::vector<DenseRun>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setEigenResult( blazemark::eigen::dmatdmatadd( N, steps ) );
...
//*************************************************************************************************
/*!\brief The main function for the dense matrix/dense matrix addition benchmark.
//
...
int main( int argc, char** argv )
{
   ...
   const std::string parameterFile( installPath + "/params/dmatdmatadd.prm" );
