...
#include <blazemark/eigen/TVec3Mat3Mult.h>
...
void estimateSteps( DenseRun& run )
{
   ...
   blaze::StaticVector<real,3UL,rowVector> vec( 0.1 );
   blaze::StaticMatrix<real,3UL,3UL,rowMajor> mat( 0.1 );
   std::vector< blaze::StaticVector<real,3UL,rowVector> > a( N, vec ), b( N );
...
//*************************************************************************************************
/*!\brief 3-dimensional transpose vector/matrix multiplication benchmark function.
//
// \param runs The specified benchmark runs.
// \param benchmarks The selection of benchmarks.
// \return void
*/
void tvec3mat3mult( std::vector<DenseRun>& runs, Benchmarks benchmarks )
{
   std::cout << std::left;
   ...
   if( benchmarks.runBlaze ) {
      ...
      for( std::vector<DenseRun>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         ...
         run->setBlazeResult( blazemark::blaze::tvec3mat3mult( N, steps ) );
   ...
   if( benchmarks.runBoost ) {
      ...
      for( std::vector<DenseRun>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setBoostResult( blazemark::boost::tvec3mat3mult( N, steps ) );
   ...
   if( benchmarks.runBlitz ) {
      ...
      for( std::vector<DenseRun>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setBlitzResult( blazemark::blitz::tvec3mat3mult( N, steps ) );
   ...
   if( benchmarks.runEigen ) {
      ...
      for( std::vector<DenseRun>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setEigenResult( blazemark::eigen::tvec3mat3mult( N, steps ) );
...
//*************************************************************************************************
/*!\brief The main function for the 3-dimensional transpose vector/matrix multiplication benchmark.
//
...
int main( int argc, char** argv )
{
   ...
   const std::string parameterFile( installPath + "/params/tvec3mat3mult.prm" );
