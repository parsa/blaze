...
//
// This type definition specifies the type of a single benchmark run for the 3D matrix/matrix
...
// \return void
*/
void mat3mat3mult( std::vector<Run>& runs, Benchmarks benchmarks )
{
   std::cout << std::left;
   ...
   if( benchmarks.runFLENS ) {
      ...
      for( std::vector<Run>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setFLENSResult( blazemark::flens::mat3mat3mult( N, steps ) );
