...
//  \brief Source file for the dense matrix/transpose dense matrix multiplication benchmark
//
...
#include <blazemark/blas/DMatTDMatMult.h>
#include <blazemark/blaze/DMatTDMatMult.h>
#include <blazemark/boost/DMatTDMatMult.h>
#include <blazemark/eigen/DMatTDMatMult.h>
#include <blazemark/flens/DMatTDMatMult.h>
#include <blazemark/mtl/DMatTDMatMult.h>
...
//
// This type definition specifies the type of a single benchmark run for the dense
...
void estimateSteps( Run& run )
{
   ...
   blaze::DynamicMatrix<element_t,rowMajor> A( N, N, 0.1 ), C( N, N );
   blaze::DynamicMatrix<element_t,columnMajor> B( N, N, 0.1 );
...
//*************************************************************************************************
/*!\brief Dense matrix/transpose dense matrix multiplication benchmark function.
//
// \param runs The specified benchmark runs.
// \param benchmarks The selection of benchmarks.
// \return void
*/
void dmattdmatmult( std::vector<Run>& runs, Benchmarks benchmarks )
{
   std::cout << std::left;
   ...
   if( benchmarks.runBLAS ) {
      ...
      for( std::vector<Run>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         ...
         run->setBLASResult( blazemark::blas::dmattdmatmult( N, steps ) );
   ...
   if( benchmarks.runBlaze ) {
      ...
      for( std::vector<Run>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setBlazeResult( blazemark::blaze::dmattdmatmult( N, steps ) );
   ...
   if( benchmarks.runBoost ) {
      ...
      for( std::vector<Run>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setBoostResult( blazemark::boost::dmattdmatmult( N, steps ) );
   ...
   if( benchmarks.runFLENS ) {
      ...
      for( std::vector<Run>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setFLENSResult( blazemark::flens::dmattdmatmult( N, steps ) );
   ...
   if( benchmarks.runMTL ) {
      ...
      for( std::vector<Run>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setMTLResult( blazemark::mtl::dmattdmatmult( N, steps ) );
   ...
   if( benchmarks.runEigen ) {
      ...
      for( std::vector<Run>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setEigenResult( blazemark::eigen::dmattdmatmult( N, steps ) );
...
//*************************************************************************************************
/*!\brief The main function for the dense matrix/transpose dense matrix multiplication benchmark.
//
...
int main( int argc, char** argv )
{
   ...
   const std::string parameterFile( installPath + "/params/dmattdmatmult.prm" );
   Parser<Run> parser;
   std::vector<Run> runs;
