//=================================================================================================
/*!
//  \file src/blaze/DMatDVecMult.cpp
//  \brief Source file for the dense matrix/dense vector multiplication benchmark
//
...
#include <blazemark/blas/DMatDVecMult.h>
#include <blazemark/blaze/DMatDVecMult.h>
#include <blazemark/blitz/DMatDVecMult.h>
#include <blazemark/boost/DMatDVecMult.h>
#include <blazemark/classic/DMatDVecMult.h>
#include <blazemark/eigen/DMatDVecMult.h>
#include <blazemark/flens/DMatDVecMult.h>
#include <blazemark/mtl/DMatDVecMult.h>
...
void estimateSteps( Run& run )
{
   ...
   blaze::DynamicMatrix<element_t,rowMajor> A( N, N, 0.1 );
   blaze::DynamicVector<element_t,columnVector> a( N, 0.1 ), b( N );
...
//*************************************************************************************************
/*!\brief Dense matrix/dense vector multiplication benchmark function.
//
// \param runs The specified benchmark runs.
// \param benchmarks The selection of benchmarks.
// \return void
*/
void dmatdvecmult( std::vector<Run>& runs, Benchmarks benchmarks )
{
   std::cout << std::left;
   ...
   if( benchmarks.runBlaze ) {
      ...
      for( std::vector<Run>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setBlazeResult( blazemark::blaze::dmatdvecmult( N, steps ) );
         const double mflops( ( 2U*N*N - N ) * steps / run->getBlazeResult() / 1E6 );
         std::cout << "     " << std::setw(12) << N << mflops << std::endl;
      }
   }
...
//*************************************************************************************************
/*!\brief The main function for the dense matrix/dense vector multiplication benchmark.
//
...
int main( int argc, char** argv )
{
   ...
   const std::string parameterFile( installPath + "/params/dmatdvecmult.prm" );
   Parser<Run> parser;
   std::vector<Run> runs;
