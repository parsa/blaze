...
//  \brief Source file for the transpose dense matrix/dense matrix multiplication benchmark
//
...
// matrix/dense matrix multiplication benchmark.
*/
...
void estimateSteps( Run& run )
{
   ...
   blaze::DynamicMatrix<element_t,rowMajor> B( N, N, 0.1 );
...
//*************************************************************************************************
/*!\brief Transpose dense matrix/dense matrix multiplication benchmark function.
//
// \param runs The specified benchmark runs.
// \param benchmarks The selection of benchmarks.
// \return void
*/
void tdmatdmatmult( std::vector<Run>& runs, Benchmarks benchmarks )
{
   std::cout << std::left;
   ...
   if( benchmarks.runBLAS ) {
      ...
      for( std::vector<Run>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         ...
         run->setBLASResult( blazemark::blas::tdmatdmatmult( N, steps ) );
   ...
   if( benchmarks.runBlaze ) {
      ...
      for( std::vector<Run>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setBlazeResult( blazemark::blaze::tdmatdmatmult( N, steps ) );
   ...
   if( benchmarks.runBoost ) {
      ...
      for( std::vector<Run>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setBoostResult( blazemark::boost::tdmatdmatmult( N, steps ) );
   ...
   if( benchmarks.runFLENS ) {
      ...
      for( std::vector<Run>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setFLENSResult( blazemark::flens::tdmatdmatmult( N, steps ) );
   ...
   if( benchmarks.runMTL ) {
      ...
      for( std::vector<Run>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setMTLResult( blazemark::mtl::tdmatdmatmult( N, steps ) );
   ...
   if( benchmarks.runEigen ) {
      ...
      for( std::vector<Run>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setEigenResult( blazemark::eigen::tdmatdmatmult( N, steps ) );
...
//*************************************************************************************************
/*!\brief The main function for the transpose dense matrix/dense matrix multiplication benchmark.
//
...
int main( int argc, char** argv )
{
   ...
   const std::string parameterFile( installPath + "/params/tdmatdmatmult.prm" );
   Parser<Run> parser;
   std::vector<Run> runs;
