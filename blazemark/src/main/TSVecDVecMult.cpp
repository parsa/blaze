...
//  \brief Source file for the sparse vector/dense vector inner product benchmark
//
...
//*************************************************************************************************
/*!\brief Sparse vector/dense vector inner product benchmark function.
//
// \param runs The specified benchmark runs.
// \param benchmarks The selection of benchmarks.
// \return void
*/
void tsvecdvecmult( std::vector<SparseRun>& runs, Benchmarks benchmarks )
{
   std::cout << std::left;
   ...
   if( benchmarks.runBlaze ) {
      ...
      while( run != runs.end() ) {
         ...
         for( ; run!=runs.end(); ++run ) {
            ...
            run->setBlazeResult( blazemark::blaze::tsvecdvecmult( N, F, steps ) );
            const double mflops( ( 2UL*F - 1UL ) * steps / run->getBlazeResult() / 1E6 );
   ...
   if( benchmarks.runBoost ) {
      ...
      while( run != runs.end() ) {
         ...
         for( ; run!=runs.end(); ++run ) {
            ...
            const size_t F    ( run->getNonZeros() );
            const size_t steps( run->getSteps()    );
            run->setBoostResult( blazemark::boost::tsvecdvecmult( N, F, steps ) );
            const double mflops( ( 2UL*F - 1UL ) * steps / run->getBoostResult() / 1E6 );
            std::cout << "     " << std::setw(12) << N << mflops << std::endl;
         }
      }
   }
   ...
   if( benchmarks.runGMM ) {
      ...
      while( run != runs.end() ) {
         ...
         for( ; run!=runs.end(); ++run ) {
            ...
            const size_t F    ( run->getNonZeros() );
            const size_t steps( run->getSteps()    );
            run->setGMMResult( blazemark::gmm::tsvecdvecmult( N, F, steps ) );
            const double mflops( ( 2UL*F - 1UL ) * steps / run->getGMMResult() / 1E6 );
            std::cout << "     " << std::setw(12) << N << mflops << std::endl;
         }
      }
   }
...
//*************************************************************************************************
/*!\brief The main function for the sparse vector/dense vector inner product benchmark.
//
...
int main( int argc, char** argv )
{
   ...
   const std::string parameterFile( installPath + "/params/tsvecdvecmult.prm" );
