...
//
// This type definition specifies the type of a single benchmark run for the daxpy product
// benchmark
*/
...
//*************************************************************************************************
/*!\brief Daxpy product benchmark function.
//
// \param runs The specified benchmark runs.
// \param benchmarks The selection of benchmarks.
// \return void
*/
void daxpy( std::vector<Run>& runs, Benchmarks benchmarks )
{
   std::cout << std::left;
   ...
   if( benchmarks.runClike ) {
      ...
      for( std::vector<Run>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         ...
         run->setClikeResult( blazemark::clike::daxpy( N, steps ) );
         const double mflops( ( 2UL*N ) * steps / run->getClikeResult() / 1E6 );
   ...
   if( benchmarks.runClassic ) {
      ...
      for( std::vector<Run>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setClassicResult( blazemark::classic::daxpy( N, steps ) );
         const double mflops( ( 2UL*N ) * steps / run->getClassicResult() / 1E6 );
         std::cout << "     " << std::setw(12) << N << mflops << std::endl;
      }
   }
   ...
   if( benchmarks.runBLAS ) {
      ...
      for( std::vector<Run>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setBLASResult( blazemark::blas::daxpy( N, steps ) );
         const double mflops( ( 2UL*N ) * steps / run->getBLASResult() / 1E6 );
         std::cout << "     " << std::setw(12) << N << mflops << std::endl;
      }
   }
#endif
   ...
   if( benchmarks.runBlaze ) {
      ...
      for( std::vector<Run>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setBlazeResult( blazemark::blaze::daxpy( N, steps ) );
         const double mflops( ( 2UL*N ) * steps / run->getBlazeResult() / 1E6 );
         std::cout << "     " << std::setw(12) << N << mflops << std::endl;
      }
   }
   ...
   if( benchmarks.runBoost ) {
      ...
      for( std::vector<Run>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setBoostResult( blazemark::boost::daxpy( N, steps ) );
         const double mflops( ( 2UL*N ) * steps / run->getBoostResult() / 1E6 );
         std::cout << "     " << std::setw(12) << N << mflops << std::endl;
      }
   }
   ...
   if( benchmarks.runBlitz ) {
      ...
      for( std::vector<Run>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setBlitzResult( blazemark::blitz::daxpy( N, steps ) );
         const double mflops( ( 2UL*N ) * steps / run->getBlitzResult() / 1E6 );
         std::cout << "     " << std::setw(12) << N << mflops << std::endl;
      }
   }
#endif
   ...
   if( benchmarks.runGMM ) {
      ...
      for( std::vector<Run>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setGMMResult( blazemark::gmm::daxpy( N, steps ) );
         const double mflops( ( 2UL*N ) * steps / run->getGMMResult() / 1E6 );
         std::cout << "     " << std::setw(12) << N << mflops << std::endl;
      }
   }
#endif
   ...
   if( benchmarks.runMTL ) {
      ...
      for( std::vector<Run>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setMTLResult( blazemark::mtl::daxpy( N, steps ) );
         const double mflops( ( 2UL*N ) * steps / run->getMTLResult() / 1E6 );
         std::cout << "     " << std::setw(12) << N << mflops << std::endl;
      }
   }
#endif
   ...
   if( benchmarks.runEigen ) {
      ...
      for( std::vector<Run>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setEigenResult( blazemark::eigen::daxpy( N, steps ) );
         const double mflops( ( 2UL*N ) * steps / run->getEigenResult() / 1E6 );
         std::cout << "     " << std::setw(12) << N << mflops << std::endl;
      }
   }
#endif
...
//*************************************************************************************************
/*!\brief The main function for the daxpy product benchmark.
//
...
int main( int argc, char** argv )
{
   ...
   const std::string parameterFile( installPath + "/params/daxpy.prm" );
   Parser<Run> parser;
   std::vector<Run> runs;
