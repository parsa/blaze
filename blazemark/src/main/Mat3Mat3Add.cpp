...
#include <blazemark/boost/Mat3Mat3Add.h>
...
void estimateSteps( DenseRun& run )
{
   ...
   std::vector< blaze::StaticMatrix<element_t,3UL,3UL,rowMajor> > A( N, init ), B( N, init ), C( N );
...
//*************************************************************************************************
/*!\brief 3-dimensional matrix/matrix addition benchmark function.
//
// \param runs The specified benchmark runs.
// \param benchmarks The selection of benchmarks.
// \return void
*/
void mat3mat3add( std::vector<DenseRun>& runs, Benchmarks benchmarks )
{
   std::cout << std::left;
   ...
   if( benchmarks.runBlaze ) {
      ...
      for( std::vector<DenseRun>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         ...
         run->setBlazeResult( blazemark::blaze::mat3mat3add( N, steps ) );
   ...
   if( benchmarks.runBoost ) {
      ...
      for( std::vector<DenseRun>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setBoostResult( blazemark::boost::mat3mat3add( N, steps ) );
   ...
   if( benchmarks.runBlitz ) {
      ...
      for( std::vector<DenseRun>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setBlitzResult( blazemark::blitz::mat3mat3add( N, steps ) );
   ...
   if( benchmarks.runFLENS ) {
      ...
      for( std::vector<DenseRun>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setFLENSResult( blazemark::flens::mat3mat3add( N, steps ) );
   ...
   if( benchmarks.runMTL ) {
      ...
      for( std::vector<DenseRun>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setMTLResult( blazemark::mtl::mat3mat3add( N, steps ) );
   ...
   if( benchmarks.runEigen ) {
      ...
      for( std::vector<DenseRun>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setEigenResult( blazemark::eigen::mat3mat3add( N, steps ) );
...
//*************************************************************************************************
/*!\brief The main function for the 3-dimensional matrix/matrix addition benchmark.
//
...
int main( int argc, char** argv )
{
   ...
   const std::string parameterFile( installPath + "/params/mat3mat3add.prm" );
