...
//  \brief Source file for the 6D matrix/transpose matrix multiplication benchmark
//
...
#include <blazemark/blitz/Mat6TMat6Mult.h>
#include <blazemark/boost/Mat6TMat6Mult.h>
#include <blazemark/eigen/Mat6TMat6Mult.h>
#include <blazemark/flens/Mat6TMat6Mult.h>
#include <blazemark/mtl/Mat6TMat6Mult.h>
...
//
// This type definition specifies the type of a single benchmark run for the 6D matrix/transpose
...
void estimateSteps( Run& run )
{
   ...
   blaze::StaticMatrix<element_t,6UL,6UL,rowMajor> init( 0.1 );
   std::vector< blaze::StaticMatrix<element_t,6UL,6UL,rowMajor> > A( N, init ), C( N );
   std::vector< blaze::StaticMatrix<element_t,6UL,6UL,columnMajor> > B( N, init );
...
//*************************************************************************************************
/*!\brief 6-dimensional matrix/transpose matrix multiplication benchmark function.
//
// \param runs The specified benchmark runs.
// \param benchmarks The selection of benchmarks.
// \return void
*/
void mat6tmat6mult( std::vector<Run>& runs, Benchmarks benchmarks )
{
   std::cout << std::left;
   ...
   if( benchmarks.runBlaze ) {
      ...
      for( std::vector<Run>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         ...
         run->setBlazeResult( blazemark::blaze::mat6tmat6mult( N, steps ) );
   ...
   if( benchmarks.runBoost ) {
      ...
      for( std::vector<Run>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setBoostResult( blazemark::boost::mat6tmat6mult( N, steps ) );
   ...
   if( benchmarks.runBlitz ) {
      ...
      for( std::vector<Run>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setBlitzResult( blazemark::blitz::mat6tmat6mult( N, steps ) );
   ...
   if( benchmarks.runFLENS ) {
      ...
      for( std::vector<Run>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setFLENSResult( blazemark::flens::mat6tmat6mult( N, steps ) );
   ...
   if( benchmarks.runMTL ) {
      ...
      for( std::vector<Run>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setMTLResult( blazemark::mtl::mat6tmat6mult( N, steps ) );
   ...
   if( benchmarks.runEigen ) {
      ...
      for( std::vector<Run>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setEigenResult( blazemark::eigen::mat6tmat6mult( N, steps ) );
...
//*************************************************************************************************
/*!\brief The main function for the 6-dimensional matrix/transpose matrix multiplication
//        benchmark.
//
...
int main( int argc, char** argv )
{
   ...
   const std::string parameterFile( installPath + "/params/mat6tmat6mult.prm" );
   Parser<Run> parser;
   std::vector<Run> runs;
