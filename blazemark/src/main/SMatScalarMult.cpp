...
//  \brief Source file for the sparse matrix/scalar multiplication benchmark
//
...
void estimateSteps( SparseRun& run )
{
   ...
   blaze::CompressedMatrix<real,rowMajor> A( N, N, N*F ), B( N, N );
...
//*************************************************************************************************
/*!\brief Sparse matrix/scalar multiplication benchmark function.
//
// \param runs The specified benchmark runs.
// \param benchmarks The selection of benchmarks.
// \return void
*/
void smatscalarmult( std::vector<SparseRun>& runs, Benchmarks benchmarks )
{
   std::cout << std::left;
   ...
   if( benchmarks.runBlaze ) {
      ...
      while( run != runs.end() ) {
         ...
         for( ; run!=runs.end(); ++run ) {
            ...
            run->setBlazeResult( blazemark::blaze::smatscalarmult( N, F, steps ) );
   ...
   if( benchmarks.runBoost ) {
      ...
      while( run != runs.end() ) {
         ...
         for( ; run!=runs.end(); ++run ) {
            ...
            const size_t F    ( run->getNonZeros() );
            const size_t steps( run->getSteps()    );
            run->setBoostResult( blazemark::boost::smatscalarmult( N, F, steps ) );
   ...
   if( benchmarks.runGMM ) {
      ...
      while( run != runs.end() ) {
         ...
         for( ; run!=runs.end(); ++run ) {
            ...
            const size_t F    ( run->getNonZeros() );
            const size_t steps( run->getSteps()    );
            run->setGMMResult( blazemark::gmm::smatscalarmult( N, F, steps ) );
   ...
   if( benchmarks.runMTL ) {
      ...
      while( run != runs.end() ) {
         ...
         for( ; run!=runs.end(); ++run ) {
            ...
            const size_t F    ( run->getNonZeros() );
            const size_t steps( run->getSteps()    );
            run->setMTLResult( blazemark::mtl::smatscalarmult( N, F, steps ) );
   ...
   if( benchmarks.runEigen ) {
      ...
      while( run != runs.end() ) {
         ...
         for( ; run!=runs.end(); ++run ) {
            ...
            const size_t F    ( run->getNonZeros() );
            const size_t steps( run->getSteps()    );
            run->setEigenResult( blazemark::eigen::smatscalarmult( N, F, steps ) );
...
//*************************************************************************************************
/*!\brief The main function for the sparse matrix/scalar multiplication benchmark.
//
...
int main( int argc, char** argv )
{
   ...
   const std::string parameterFile( installPath + "/params/smatscalarmult.prm" );
   ...
   try {
      smatscalarmult( runs, benchmarks );
   }
   catch( std::exception& ex ) {
