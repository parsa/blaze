//=================================================================================================
/*!
//  \file src/blaze/TSVecTSMatMult.cpp
//  \brief Source file for the transpose sparse vector/transpose sparse matrix multiplication benchmark
//
...
#include <blazemark/blaze/TSVecTSMatMult.h>
...
void estimateSteps( SparseRun& run )
{
   ...
   blaze::CompressedVector<real,rowVector> a( N, F ), b( N );
...
//*************************************************************************************************
/*!\brief Transpose sparse vector/transpose sparse matrix multiplication benchmark function.
//
// \param runs The specified benchmark runs.
// \param benchmarks The selection of benchmarks.
// \return void
*/
void tsvectsmatmult( std::vector<SparseRun>& runs, Benchmarks benchmarks )
{
   std::cout << std::left;
   ...
   if( benchmarks.runBlaze ) {
      ...
      while( run != runs.end() ) {
         ...
         for( ; run!=runs.end(); ++run ) {
            ...
            run->setBlazeResult( blazemark::blaze::tsvectsmatmult( N, F, steps ) );
   ...
   if( benchmarks.runBoost ) {
      ...
      while( run != runs.end() ) {
         ...
         for( ; run!=runs.end(); ++run ) {
            ...
            const size_t F    ( run->getNonZeros() );
            const size_t steps( run->getSteps()    );
            run->setBoostResult( blazemark::boost::tsvectsmatmult( N, F, steps ) );
...
//*************************************************************************************************
/*!\brief The main function for the transpose sparse vector/transpose sparse matrix multiplication
//        benchmark.
//
...
int main( int argc, char** argv )
{
   ...
   const std::string parameterFile( installPath + "/params/tsvectsmatmult.prm" );
   ...
   try {
      tsvectsmatmult( runs, benchmarks );
   }
   catch( std::exception& ex ) {
