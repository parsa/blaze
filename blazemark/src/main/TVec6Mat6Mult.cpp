...
//  \brief Source file for the 6D transpose vector/matrix multiplication benchmark
//
...
#include <blazemark/blaze/TVec6Mat6Mult.h>
...
void estimateSteps( DenseRun& run )
{
   ...
   blaze::StaticVector<element_t,6UL,rowVector> vec( 0.1 );
   blaze::StaticMatrix<element_t,6UL,6UL,rowMajor> mat( 0.1 );
   std::vector< blaze::StaticVector<element_t,6UL,rowVector> > a( N, vec ), b( N );
   std::vector< blaze::StaticMatrix<element_t,6UL,6UL,rowMajor> > A( N, mat );
...
//*************************************************************************************************
/*!\brief 6-dimensional transpose vector/matrix multiplication benchmark function.
//
// \param runs The specified benchmark runs.
// \param benchmarks The selection of benchmarks.
// \return void
*/
void tvec6mat6mult( std::vector<DenseRun>& runs, Benchmarks benchmarks )
{
   std::cout << std::left;
   ...
   if( benchmarks.runBlaze ) {
      ...
      for( std::vector<DenseRun>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         ...
         run->setBlazeResult( blazemark::blaze::tvec6mat6mult( N, steps ) );
         const double mflops( 66UL * steps / run->getBlazeResult() / 1E6 );
   ...
   if( benchmarks.runBoost ) {
      ...
      for( std::vector<DenseRun>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setBoostResult( blazemark::boost::tvec6mat6mult( N, steps ) );
         const double mflops( 66UL * steps / run->getBoostResult() / 1E6 );
         std::cout << "     " << std::setw(12) << N << mflops << std::endl;
      }
   }
   ...
   if( benchmarks.runEigen ) {
      ...
      for( std::vector<DenseRun>::iterator run=runs.begin(); run!=runs.end(); ++run ) {
         const size_t N    ( run->getSize()  );
         const size_t steps( run->getSteps() );
         run->setEigenResult( blazemark::eigen::tvec6mat6mult( N, steps ) );
         const double mflops( 66UL * steps / run->getEigenResult() / 1E6 );
         std::cout << "     " << std::setw(12) << N << mflops << std::endl;
      }
   }
#endif
...
//*************************************************************************************************
/*!\brief The main function for the 6-dimensional transpose vector/matrix multiplication benchmark.
//
...
int main( int argc, char** argv )
{
   ...
   const std::string parameterFile( installPath + "/params/tvec6mat6mult.prm" );
