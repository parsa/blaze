   ...
   try
   {
      ...
      for( size_t i=0UL; i<=9UL; ++i ) {
         for( size_t j=0UL; j<=9UL; ++j ) {
            RUN_DMATDMATADD_TEST( CMDb( i, j ), CMDb( i, j ) );
         }
      }

      // Running tests with large matrices
      RUN_DMATDMATADD_TEST( CMDb(  67UL,  67UL ), CMDb(  67UL,  67UL ) );
      RUN_DMATDMATADD_TEST( CMDb(  67UL, 127UL ), CMDb(  67UL, 127UL ) );
      RUN_DMATDMATADD_TEST( CMDb( 128UL,  64UL ), CMDb( 128UL,  64UL ) );
      RUN_DMATDMATADD_TEST( CMDb( 128UL, 128UL ), CMDb( 128UL, 128UL ) );
   }
