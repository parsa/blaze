      ...
      for( size_t i=0UL; i<=6UL; ++i ) {
         for( size_t j=0UL; j<=6UL; ++j ) {
            for( size_t k=0UL; k<=i*j; ++k ) {
               RUN_DMATSMATADD_TEST( CMDa( i, j ), CMCb( i, j, k ) );
            }
         }
      }

      // Running tests with large matrices
      RUN_DMATSMATADD_TEST( CMDa(  67UL,  67UL ), CMCb(  67UL,  67UL,  7UL ) );
      RUN_DMATSMATADD_TEST( CMDa(  67UL, 127UL ), CMCb(  67UL, 127UL, 13UL ) );
      RUN_DMATSMATADD_TEST( CMDa( 128UL,  64UL ), CMCb( 128UL,  64UL,  8UL ) );
      RUN_DMATSMATADD_TEST( CMDa( 128UL, 128UL ), CMCb( 128UL, 128UL, 16UL ) );
   }
